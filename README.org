...
doc-capture 使用 Org-mode 的 =org-file-apps= 配置来打开文档。你需要配置文档查看器的页码跳转：
...
;; 或配置 Okular
(add-to-list 'org-file-apps '("\\.pdf::\\([0-9]+\\)\\'" . "okular %s --page %1"))

;; 配置其他格式（如 EPUB）
(add-to-list 'org-file-apps '("\\.epub::\\([0-9]+\\)\\'" . "zathura %s -P %1"))
#+end_src

*注意*：=%s= 代表文件路径，=%1= 代表页码（从正则表达式捕获组获取）。
