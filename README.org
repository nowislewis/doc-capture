...
- 通过 org-file-apps 配置文档查看器（推荐 Zathura, Okular）
...
doc-capture 会自动配置所有支持格式的页码跳转功能。

** 自动配置（推荐）

默认使用 Zathura 作为文档查看器，支持所有格式的页码跳转：

#+begin_src elisp
;; 加载 doc-capture（已自动配置 Zathura）
(require 'doc-capture)
#+end_src

支持的格式：PDF, EPUB, DJVU, XPS, CBZ, CBR, MOBI, AZW, AZW3, FB2, PS
...
#+begin_src elisp
;; 使用 Okular
(setq doc-capture-viewer-command "okular %s --page %1")

;; 使用 Evince
(setq doc-capture-viewer-command "evince -p %1 %s")

;; 使用 MuPDF
(setq doc-capture-viewer-command "mupdf %s %1")

;; 使用系统默认程序（不支持页码跳转）
(setq doc-capture-viewer-command nil)

(require 'doc-capture)
#+end_src

*注意*：=%s= 代表文件路径，=%1= 代表页码。需要在 =(require 'doc-capture)= *之前*设置。
...
;; 自定义文档查看器（可选，默认 Zathura）
(setq doc-capture-viewer-command "zathura %s -P %1")  ; 默认值
...
;; 添加或移除支持的格式（可选）
(setq doc-capture-supported-extensions
...
[[file:/path/to/document.pdf::42][document.pdf]]
#+end_example
...
[[file:/path/to/document.pdf::42][document.pdf - 第42页]]
#+end_example
...
[[file:/path/to/example.pdf::5][example.pdf]]
#+end_example
